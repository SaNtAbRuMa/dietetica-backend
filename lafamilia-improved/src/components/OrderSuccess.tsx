...
const WHATSAPP_NUMBER = '5492284322581';

export function OrderSuccess({ onBackToStore, orderNumber }: OrderSuccessProps) {
  // SEGURIDAD: Ya no generamos un número al azar. Si no hay orden, es un error crítico.
  if (!orderNumber) {
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-4 py-12">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center border border-stone-100">
           ...
           <p className="text-stone-500 mb-6">No se pudo recuperar el número de tu pedido. Por favor, contactanos por WhatsApp.</p>
           <button onClick={onBackToStore} className="w-full py-3 bg-stone-100 text-stone-700 rounded-xl font-medium">Volver a la tienda</button>
        </div>
      </div>
    );
  }
    ...
    >
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 text-center border border-stone-100">
        ...
        <p className="text-stone-500 mb-6 leading-relaxed text-sm">
          Gracias por elegirnos. Nos comunicaremos a la brevedad para coordinar el pago y la entrega de tu pedido.
        </p>

        <div className="bg-stone-50 rounded-2xl p-4 mb-6 text-left">
          <p className="text-xs text-stone-400 mb-1 uppercase tracking-wide font-medium">Número de orden</p>
          <p className="font-mono font-bold text-stone-900 text-xl">{orderNumber}</p>
        </div>

        <a
          href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(`Hola! Acabo de realizar el pedido ${orderNumber}. ¿Me pueden confirmar?`)}`}
          target="_blank"
